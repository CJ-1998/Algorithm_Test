...
vector<int> solution(vector<vector<int>> dice) {
    ...
    int len=dice.size();
    int prob=fac(len)/fac(len/2)/fac(len/2)/2;
    ...
    for(int i=0;i<choose.size()/2;i++){
        ...
        vector<int> b=choose[choose.size()-1-i];
    ...
    for(int i=0;i<prob;i++){
        record[prob*2-1-i][0]=record[i][2];
        record[prob*2-1-i][1]=record[i][1];
        record[prob*2-1-i][2]=record[i][0];
    }

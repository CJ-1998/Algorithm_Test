...
vector<int> solution(vector<string> id_list, vector<string> report, int k) {
    ...
    for(int i=0;i<id_list.size();i++){
        vector<string> temp;
        m.insert({id_list[i],temp});
        name.insert({id_list[i],0});
    }
    ...
    for(int i=0;i<report.size();i++){
        ...
        while(stream>>t){
            tempName.push_back(t);
        }
        
        if(find(m[tempName[1]].begin(),m[tempName[1]].end(),tempName[0])==m[tempName[1]].end())
            m[tempName[1]].push_back(tempName[0]);
    }
    ...
    for(int i=0;i<id_list.size();i++){
        answer.push_back(name[id_list[i]]);
    }
...
풀이 : 
    map에 신고한 사람 목록 넣어둠
    그 목록 길이 k 넘으면 아래 진행함
    목록에 있는 사람이 key인 map에 메일 받는 횟수 올림
*/
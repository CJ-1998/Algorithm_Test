...
bool sorting(vector<string>a, vector<string>b){
...
vector<int> solution(string s) {
    ...
    sort(tuple.begin(),tuple.end(),sorting);
    
    vector<int> answer;
    answer.push_back(stoi(tuple[0][0]));
    
    for(int i=1;i<tuple.size();i++){
        for(int j=0;j<tuple[i].size();j++){
            if(find(answer.begin(),answer.end(),stoi(tuple[i][j]))==answer.end()){
